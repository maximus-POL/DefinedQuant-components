# Legacy governed plan compiler

**Status:** Phase-1 compile-only contract implemented; governed execution remains deferred
...
Phase 1 may make only these scoped positive claims for a successfully compiled plan:
...
Provider
: An external system or organization behind an implementation. It may supply data, calculation,
  or both; a provider is not itself the method or capability.
...
Component
: The existing public compatibility package, which colocates a financial contract and one
  DQ-native implementation. Current component tools, operation records, and `subject_hash`
  semantics remain unchanged. A component is not the future abstraction for every possible
...
Current components remain the public execution and discovery compatibility surface. The current
Phase-1 bridge projects a selected component into:
...
The bridge currently creates only the DQ-native implementation above. Alternative registered
implementations can participate in deterministic compilation when supplied through an explicit
governed registry, but Phase 1 does not invoke their adapters or transports.
...
Existing `subject_hash` values, protocol vectors, all eight MCP tool schemas, operation behavior, and
